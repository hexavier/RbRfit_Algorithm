...
metabolites_sd = pd.read_excel(data_dir+"metabolites.xlsx",sheet_name=1,index_col="name")
proteins = pd.read_excel(data_dir+"proteins.xlsx",index_col="name")
proteins_sd = pd.read_excel(data_dir+"proteins.xlsx",sheet_name=1,index_col="name")
...
summary = arf.define_reactions(rxn_id,model,fluxes,proteins,metabolites,proteins_sd,metabolites_sd,binding_site=binding_site)
candidates = arf.define_candidates(rxn_id,reg_coli,metabolites,metabolites_sd)
markov_par = {'freq':2,'nrecord':20,'burn_in':0} # Record once every 20 samples, 200 samples, skip 0 first samples
e = time()
