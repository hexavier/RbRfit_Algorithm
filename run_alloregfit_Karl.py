...
markov_par = {'freq':1,'nrecord':2,'burn_in':0} # Record once every 20 samples, 200 samples, skip 0 first samples
e = time()
print('Load summary and candidates: %fs' % (e-s))

#%% Run fit with regulators
s = time()
results = arf.fit_reactions(summary,model,markov_par,metabolites,metabolites_sd)#,candidates,maxreg=1,coop=True)
e = time()
